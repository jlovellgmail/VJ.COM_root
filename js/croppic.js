			...
			that.form.find('input[type="file"]').change(function(){
				...
				$.ajax({
					...
					if(response.status=='success'){
						...
						var urlToShow = response.url.replace(/\\/gi, "/");
						if (that.imgInitW > that.objW || that.imgInitH > that.objH)
						{
							that.obj.append('<img src="'+urlToShow+'">');
						}
						that.initCropper();
			...
			$.ajax({
                ...
                type: 'POST'
					...
					if(response.status=='success'){
						...
						that.destroy();
						var urlToShow = response.url.replace(/\\/gi, "/");
						that.obj.append('<img class="'+ that.id + '_croppedImg" src="'+urlToShow+'">');
						if(that.options.outputUrlId !== ''){	$('#'+that.options.outputUrlId).val(urlToShow);	}
						
						that.croppedImg = that.obj.find('.'+ that.id + '_croppedImg');

						that.init();
						
						that.hideLoader();
						
						that.loadedBeforeCrop = false;
